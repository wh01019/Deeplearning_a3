...
from keras.layers import Dense, Dropout, LSTM, SimpleRNN, GRU, Input
...
def mda(y_true, y_pred, t=12):
    """
    Mean Directional Accuracy (MDA) calculation using TensorFlow.
    
    Args:
        y_true (tf.Tensor): The true values.
        y_pred (tf.Tensor): The predicted values.
        t (int): The lag used to compute the directional change.
        
    Returns:
        tf.Tensor: The mean directional accuracy.
    """
    # Calculate the directional change for y_true and y_pred
    y_true_diff = tf.sign(y_true[t:] - y_true[:-t])
    y_pred_diff = tf.sign(y_pred[t:] - y_pred[:-t])
    
    # Compare directional changes
    direction_match = tf.equal(y_true_diff, y_pred_diff)
    
    # Compute the mean directional accuracy
    mda_value = tf.reduce_mean(tf.cast(direction_match, tf.float32))
...
def build_SimpleRNN(input_shape, parameters):
    model = Sequential()
    model.add(Input(batch_shape=input_shape))  
    model.add(SimpleRNN(units=parameters['RNN_size'], unroll=True, stateful=True))
...
def build_GRU(input_shape, parameters):
    model = Sequential()
    model.add(Input(batch_shape=input_shape))  # (batch_size, timesteps, features)
    model.add(GRU(units=parameters['RNN_size'], unroll=True, stateful=True))
    model.add(Dropout(parameters['dropout']))
    model.add(Dense(parameters['FC_size'], activation='relu'))
    model.add(Dense(1))
    optimizer = parameters["optim"](learning_rate = parameters["lr"])
    ...
    def __init__(self,
        ...
        if type(self.__X) is list:
            ...
            for x, y in zip(self.__X, self.__y):
                ...
                #                      index = true.index)      
                pred_org = pd.Series(self._destnd(pred.values.reshape(-1, 1),
                                  self.__raw_y.reshape(-1, 1),
                                  self.__stnd_method).reshape(-1),
                                  index=pred.index)
                true_org = pd.Series(self._destnd(true.values.reshape(-1, 1),
                                                self.__raw_y.reshape(-1, 1),
                                                self.__stnd_method).reshape(-1),
            ...
            self.predictions_org = pd.Series(self._destnd(self.predictions.values.reshape(-1,1),
                                                          ...
                                                          self.__stnd_method)
                                                 .reshape(-1))
            self.true_org = pd.Series(self._destnd(self.__y.reshape(-1,1),
                                                   self.__raw_y.reshape(-1, 1),
                                                   self.__stnd_method)
                                          .reshape(-1))     
